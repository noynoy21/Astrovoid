...
#define WIDTH 20
#define HEIGHT 15
#define MAX_BULLETS_PER_PLAYER 3  // Each player gets 3 bullets
#define MAX_LIVES 3
...
void playPVP() {
    ...
    Entity player1 = { WIDTH / 4, HEIGHT - 2, 1 };
    Entity player2 = { 3 * WIDTH / 4, 2, 1 };
    PVPBullet bullets[MAX_BULLETS_PER_PLAYER * 2];  // Total bullets for both players
    
    int lives1 = MAX_LIVES;
    int lives2 = MAX_LIVES;
    int score1 = 0;
    int score2 = 0;
    ...
    // Initialize bullets
    for (int i = 0; i < MAX_BULLETS_PER_PLAYER * 2; i++) {
        bullets[i].active = 0;
        bullets[i].owner = 0;
    }
    ...
    while (1) {
        ...
        if (_kbhit()) {
            ...
            if (key == 'a' || key == 'A') player1.x--;
            if (key == 'd' || key == 'D') player1.x++;
            ...
            if (key == 32) {  // Spacebar
                ...
                // Only fire if Player 1 has less than MAX_BULLETS_PER_PLAYER active bullets
                if (p1ActiveBullets < MAX_BULLETS_PER_PLAYER) {
                    for (int i = 0; i < MAX_BULLETS_PER_PLAYER * 2; i++) {
                        if (!bullets[i].active) {
            ...
            if (key == 224) {  // Arrow key prefix
                key = _getch();
                if (key == 75) player2.x--;      // Left arrow
                if (key == 77) player2.x++;      // Right arrow
                if (key == 72) player2.y--;      // Up arrow
                if (key == 80) player2.y++;      // Down arrow
            }
            
            // Enter for Player 2 shooting
            if (key == 13) {  // Enter key
                ...
                // Only fire if Player 2 has less than MAX_BULLETS_PER_PLAYER active bullets
                if (p2ActiveBullets < MAX_BULLETS_PER_PLAYER) {
                    for (int i = 0; i < MAX_BULLETS_PER_PLAYER * 2; i++) {
                        if (!bullets[i].active) {
                            bullets[i].x = player2.x;
                            bullets[i].y = player2.y + 1;
                            bullets[i].active = 1;
                            bullets[i].owner = 2;
                            break;
                        }
        ...
        // Boundary checking
        if (player1.x < 1) player1.x = 1;
        if (player1.x > WIDTH - 2) player1.x = WIDTH - 2;
        if (player1.y < 1) player1.y = 1;
        if (player1.y > HEIGHT - 2) player1.y = HEIGHT - 2;
        
        if (player2.x < 1) player2.x = 1;
        if (player2.x > WIDTH - 2) player2.x = WIDTH - 2;
        if (player2.y < 1) player2.y = 1;
        if (player2.y > HEIGHT - 2) player2.y = HEIGHT - 2;

        // Move bullets
        for (int i = 0; i < MAX_BULLETS_PER_PLAYER * 2; i++) {
            if (bullets[i].active) {
                if (bullets[i].owner == 1) {
                    bullets[i].y--;  // Player1 bullets go up
                    if (bullets[i].y < 1)
                        bullets[i].active = 0;
                } else if (bullets[i].owner == 2) {
                    bullets[i].y++;  // Player2 bullets go down
                    if (bullets[i].y > HEIGHT - 1)
                        bullets[i].active = 0;
                }
            }
        }
        ...
        printf("\u25B2");  // Blue up arrow for P1
        
        setColor(10);
        gotoxy(player2.x, player2.y);
        printf("\u25BC");  // Green down arrow for P2
        ...
        int p1AvailableBullets = MAX_BULLETS_PER_PLAYER - p1ActiveBullets;
        int p2AvailableBullets = MAX_BULLETS_PER_PLAYER - p2ActiveBullets;
        
        // Ensure bullet counts are valid
        if (p1AvailableBullets < 0) p1AvailableBullets = 0;
        if (p2AvailableBullets < 0) p2AvailableBullets = 0;
        if (p1AvailableBullets > MAX_BULLETS_PER_PLAYER) p1AvailableBullets = MAX_BULLETS_PER_PLAYER;
        if (p2AvailableBullets > MAX_BULLETS_PER_PLAYER) p2AvailableBullets = MAX_BULLETS_PER_PLAYER;
        ...
        for (int i = p1AvailableBullets; i < MAX_BULLETS_PER_PLAYER; i++) printf(" ");
        ...
        for (int i = p2AvailableBullets; i < MAX_BULLETS_PER_PLAYER; i++) printf(" ");
